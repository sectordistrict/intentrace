                        ...
                        if eph_return.is_ok() {
                            ...
                            if getting_current_break {
                                ...
                                if mem_difference == 0 {
                                    ...
                                    self.write_text(
                                        mem_difference_bytes.to_string().custom_color(*OUR_YELLOW),
                                    );
                                } else {
                    ...
                    Entering => {
                        match whence {
                            ...
                            Whence::SeekData => {
                                ...
                                self.write_path_file(filename);
                                self.general_text(" to ");
                                self.write_text("the nearest data block".custom_color(*OUR_YELLOW));
                            ...
                            Whence::SeekHole => {
                                ...
                                self.write_path_file(filename);
                                self.general_text(" to ");
                                self.write_text("the nearest data hole".custom_color(*OUR_YELLOW));
                                self.general_text(" you find ");
                                if offset_num == 0 {
                                    self.write_text(
                                        "at the beginning of the file".custom_color(*OUR_YELLOW),
                ...
                match self.state {
                    Entering => {
                        if !set_struct.is_null() {
                            ...
                            if pid_of_self {
                                self.write_text("the calling process's".custom_color(*OUR_YELLOW));
                            } else {
                                self.write_text("process ".custom_color(*OUR_YELLOW));
                ...
                match self.state {
                    Entering => {
                        if !cpus.is_empty() {
                            ...
                            for cpu in cpu_iter {
                                self.write_text(", ".custom_color(*OUR_YELLOW));
                                self.write_text(format!("[CPU {}]", cpu).custom_color(*OUR_YELLOW));
                            }
                        } else {
                ...
                match self.state {
                    Entering => {
                        ...
                        if options.contains(WaitPidFlag::WCONTINUED) {
                            options_ticked.push("is resumed by SIGCONT".custom_color(*OUR_YELLOW));
                        }
...
impl SyscallObject {
    ...
    pub(crate) fn possible_dirfd_file(&mut self, dirfd: i32, filename: String) {
        ...
        if file_path_buf.is_relative() {
            if dirfd == AT_FDCWD {
                ...
                self.write_text(
                    current_working_directory.as_path().to_string_lossy().custom_color(*OUR_YELLOW),
                );
                self.write_text("/".custom_color(*OUR_YELLOW));
    ...
    pub fn resource_matcher(&mut self, resource: Resource) {
        match resource {
            ...
            Resource::RLIMIT_NOFILE => {
                self.write_text("maximum allowed open file descriptors".custom_color(*OUR_YELLOW));
            }
            Resource::RLIMIT_STACK => {

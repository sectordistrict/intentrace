...
pub fn initialize_skeletons_map() -> HashMap<Sysno, Syscall_Shape> {
    ...
    let array: Vec<(Sysno, Syscall_Shape)> = vec![
        ...
        // open a file (can also create a file)
        // open considers relative paths provided as relative to CWD
        (
            Sysno::open,
            Syscall_Shape {
                // types: &[
        ...
        (
            ...
            // the system call was renamed in from pread() to pread64().
            // syscall number remains the same
        ...
        // old and new descriptors may be used interchangeably afterwards
        (
            Sysno::dup,
            Syscall_Shape {
                // types: &[File_Descriptor("")],
        ...
        // it closes and overrwrites the newfd if its used
        (
            Sysno::dup2,
            Syscall_Shape {
        ...
        // same as dup2 but the caller can close-on-exec the new file descriptor
        (
            Sysno::dup3,
            Syscall_Shape {
                // types: &[
        ...
        // waiting until one or more of the
        ...
        // This is useful when a process needs access to the PID of another process
        // to send signals, retrieve information about the process, etc
        // It can also be used to monitor the lifetime of the process
        // (the pidfd is closed when the process terminates)
        (
        ...
        (
            // process's open file descriptors are closed
            // children are inherited by init(1)
        ...
        (
            Sysno::clone3,
            Syscall_Shape {
                // the size argument is for forwards compatibility
        ...
        (
            ...
            Syscall_Shape {
                ...
                //     Address,
                //     Pointer_To_Numeric(None), // pointer to int
                //     Pointer_To_Numeric(None), // pointer to int
                //     Unsigned_Numeric,
                // ],
                syscall_return: Numeric_Or_Errno,
        ...
        (
            ...
            //      created to avoid the old fork() behaviour
            //      of copying everything from the parent process
            //      its similar to fork but doesnt copy the address space
            //
            // 2- the parent's execution stops until the child
        ...
        (
            ...
            // given that they are in user-space, the kernel can't do anything in case a thread dies while holding a lock,
            // in that case the only way for waiting threads to be stopped is by rebooting!
            // to get around this, in linux, whever a thread exits (any thread) the kernel checks if it has a robust futex list
